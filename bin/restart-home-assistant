...
header "Restart Home Assistant Container" 6
header "--------------------------------" 6
printf "\n"
...
header "Restarting docker container..." 6

if $DOCKER restart $NAME; then
  header "Restarted docker container." 2 
else
  header "Failed to restart docker container." 1
  exit 1
fi
